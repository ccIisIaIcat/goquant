...
def create_distplot(
    ...
    **this function is deprecated**, use instead :mod:`plotly.express`
    ...
    validate_equal_length(hist_data, group_labels)
...
class _Distplot(object):
    ...
    def make_kde(self):
        ...
        for index in range(self.trace_number):
            ...
            self.curve_y[index] = st.gaussian_kde(self.hist_data[index])(
                self.curve_x[index]
            )
    ...
    def make_normal(self):
        ...
        for index in range(self.trace_number):
            mean[index], sd[index] = st.norm.fit(self.hist_data[index])
            self.curve_x[index] = [
                self.start[index] + x * (self.end[index] - self.start[index]) / 500
                for x in range(500)
            ]
            self.curve_y[index] = st.norm.pdf(
